...
def resizeImage(img):
    ...
    save_image(img, join_dir(cwd, 'Temp.png'))
...
def get_predicted_class():
    ...
    prediction = model.predict([gray_image.reshape(56, 100, 1)])
    return np.argmax(prediction), (np.amax(prediction) / (prediction[0][0] + prediction[0][1] + prediction[0][2]))


def show_statistics(predictedClass, confidence):
...
# Call the control car method to start controlling the MountainCar through hand gestures
control_car()
